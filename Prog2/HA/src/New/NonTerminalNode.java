...
public class NonTerminalNode extends Node {
	...
	public static Node parse( StringBuilder nodeRepr ) {
		...
		Matcher opening = Pattern.compile( "([A-Z]+):\\(" ).matcher( nodeRepr );
		Matcher closing = Pattern.compile( "[A-Za-z:\\s()]+\\s(\\))" ).matcher( nodeRepr );
		...
		// to test if you have a non-terminal
		 ...
		 if (opening.matches() && closing.matches()){
			...
			while (nodeRepr.length()>0){
				...
				Matcher term = Pattern.compile( "([A-Z]+):([A-Za-z]+)" ).matcher( nodeRepr );
